...
if(!empty($_POST["cat_id"])){
    //Fetch all state data
      $query =mysqli_query($con,"SELECT * FROM vehicle WHERE Id=".$_POST['cat_id']);
  ...
  //  $rowCount = $query->num_rows;
   ...
   // if($rowCount > 0){
    if(mysqli_num_rows($query)>0)
       {
        echo '<option name="cat_id" >Select cat</option>';
       // while($row = $query->fetch_assoc()){ 
              while($row=mysqli_fetch_assoc($query)){
                  
            echo '<option name="cat_id" value="'.$row['cat'].'">'.$row['cat'].'</option>';
        }
    }else{
...
elseif(!empty($_POST["vehicle_id"])){
    //Fetch all city data
  //  $query1 = $db->query("SELECT * FROM cities WHERE state_id = ".$_POST['state_id']." AND status = 1 ORDER BY city_name ASC");
      $query1 =mysqli_query($con,"SELECT * FROM vehicle WHERE vehicle ='".$_POST['vehicle_id']."'");
    //Count total number of rows
  //  $rowCount = $query->num_rows;
     $rowcount1=mysqli_num_rows($query1);
    //City option list
