...
if(isset($_POST['addsubmit']))
{
    ...
    $batch = $_POST['trade'];
    $batch11 = $_POST['requirement'];

$date=date("Y/m/d");
  $q = mysqli_query($con,"UPDATE `manpower_requirement` SET `Unitcat`='$category1',`Unit`='$category2',`vehicle`='$category3',`batch`='$Brigade2',`trade`='$batch',`requirement`='$batch11',`date`='$date' WHERE Id='$ii'");
  //echo "UPDATE `manpower_link` SET `cat_unit`='$category1',`Unit`='$category2',`vehicle`='$category3',`batch`='$Brigade2',`trade`='$batch',`name`='$batch11',`date`='$date' WHERE Id='$ii'";die();
   ...
   if($q){
    echo '<script>alert("Update Successfully.");window.location.assign("manpower_requirement.php");</script>';
  }
  else{
      echo '<script>alert("please Add Again....");window.location.assign("edit_manpower_requirement.php");</script>';
  }
}
?> 
...
$date=date("Y/m/d");
  $q = mysqli_query($con,"SELECT 
                trade.Id,
                manpower_requirement.trade as trade_id,
                manpower_requirement.id,
                manpower_requirement.Unitcat as Unitcat_id,
                unitcat.Unitcat,
                manpower_requirement.Unit as Unit_id,
                Brigade.Brigade,
                vehicle.vehicle,
                manpower_requirement.vehicle as vehicle_id,
                vehicle.batch,
                manpower_requirement.requirement,
                manpower_requirement.date,
                trade.trade From manpower_requirement 
                LEFT JOIN trade ON trade.Id=manpower_requirement.trade
                LEFT JOIN unitcat ON manpower_requirement.Unitcat=unitcat.id
                LEFT JOIN Brigade ON manpower_requirement.Unit=Brigade.Id
                LEFT JOIN vehicle ON manpower_requirement.vehicle=vehicle.Id
   Where manpower_requirement.id='$unitcat'");
...
<select id="batch" class="form-control" name="batch">
 ...
 <div class="col-sm-10">
             <input type="text" value="<?php echo $name; ?>" name="requirement">
</div>
</div>
