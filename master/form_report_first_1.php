        ...
        <div class="panel-body table-responsive">
          ...
          //  "SELECT * From fault,Product,subproduct where fault.product =Product.Product and subproduct.product = Product.Id and Brigade.Id=batch.unit";
             ...
             //    $query=mysqli_query($con,"select recovery.unit,recoverytime.Id,recovery.Brigade,recoverytime.vehicle,Product.Product,recoverytime.timelycompletion from recovery INNER JOIN Product ON recovery.product=Product.Id INNER JOIN recoverytime ON recoverytime.recoveryId=recovery.Id and recoverytime.timelycompletion='pending' AND recoverytime.recoveryId='$id'") ;
               //  $query=mysqli_query($con,"select recovery.Brigade,recovery.Id from recoverytime INNER JOIN recovery ON recoverytime.recoveryId=recovery.Id INNER JOIN Product ON recovery.product=Product.Id and recoverytime.recoveryId='$id' and recoverytime.timelycompletion='pending'") ;                    
             //  select recovery.Brigade,recovery.Id,count(recoverytime.timelycompletion) from recoverytime INNER JOIN recovery ON recoverytime.recoveryId=recovery.Id and recovery.Unit='CCA' and recoverytime.timelycompletion='pending'
                //  select manpower_link.trade,trade.trade from manpower_link INNER JOIN trade ON manpower_link.trade=trade.Id and manpower_link.cat_unit='TTP'
                 // $query=mysqli_query($con,"select recovery.Brigade,recovery.Id,count(recoverytime.timelycompletion) from recoverytime INNER JOIN recovery ON recoverytime.recoveryId=recovery.Id INNER JOIN Product ON recovery.product=Product.Id and recovery.Unit='$id' and recoverytime.timelycompletion='pending'") ;
                  $query=mysqli_query($con,"select recovery.Brigade,recovery.Id,count(recoverytime.timelycompletion) from recoverytime INNER JOIN recovery ON recoverytime.recoveryId=recovery.Id and recovery.Unit='$id' and recoverytime.timelycompletion='pending'"); 
...
//$sql="select count(name),Unit from manpower_link where Unit IN (select distinct(Unit) from manpower_link where cat_unit='CCRP2' and Unit='Z');";
              ...
              //  while ($row=mysqli_fetch_row($result))
               //   {
                    ?>
                      <tr>
                       <!--   <td><a href="form_report_first_1.php?id=<?php //echo $res['1']?>"><?php// echo $res['unit']?></a></td>-->
                        <td><a href="form_report_first_2.php?id=<?php echo $res['0']?>"><?php echo $res[0]?></a></td>
