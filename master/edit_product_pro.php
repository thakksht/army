...
$date=date("Y/m/d");
  $q = mysqli_query($con,"UPDATE `product` SET `catprod`='$Unitcategory', `Product`='$Brigade2', `Date`='$date' WHERE Id='$ii'");
//echo "UPDATE brigade SET unitcategory='$Unitcategory' and Brigade='$Brigade2' WHERE Id='$ii'";die();
   if($q){
    echo '<script>alert("Update Successfully.");window.location.assign("manage_product.php");</script>';
      ...
      echo '<script>alert("please Add Again....");window.location.assign("edit_product_pro.php");</script>';
  }
}
?> 
...
$date=date("Y/m/d");
  $q = mysqli_query($con,"SELECT * from product Where Id='$unitcat'");
  
  $res=mysqli_fetch_array($q);
$Id = $res['Id'];
$Brigade = $res['Product'];
$unitcategory = $res['catprod'];
$qq = mysqli_query($con,"SELECT cat_prod from category_product Where Id='$unitcategory'");
$row=mysqli_fetch_array($qq);
$unitcat = $row['cat_prod'];
?>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                   ...
                   <option value="<?php echo $unitcategory ?>"><?php echo $unitcat ?></option>
                    ...
                    $qu1=mysqli_query($con,"select * from category_product where isactive = 1 and cat_prod!='$unitcat'") ;                    
