  ...
  <div class="page-header">
    <h1 class="title">Batch</h1>
      <ol class="breadcrumb">
        ...
        <li class="active">Manpower Requirement</li>
      </ol>
...
//echo "INSERT INTO `recovery`(`unit`, `vehicle`,`product`, `natureCAS`, `timeCAS`, `typerecovery`, `REC`, `timecompletion`, `GR`, `remark`, `isactive`) values('$unit','$vehicle','$product','$natureCAS','$timeCAS','$typerecovery','$REC','$timecompletion','$GR','$remark','$isactive')";
 //die("fgfd");
  $q = mysqli_query($con,"INSERT INTO `manpower_requirement`(`Unitcat`,  `date`,`Unit`,`vehicle`,`batch`, `trade`, `requirement`, `isactive`) values('$unit','$date','$category','$vehicle','$batch','$trade','$name','1')");
  if($q){
    echo '<script>alert("Add Successfully.");window.location.assign("manpower_requirement.php");</script>';
  }
  else{
      echo '<script>alert("please Add Again....");window.location.assign("manpower_requirement_add.php");</script>';
  }
}
...
<select id="country"  class="form-control" onchange="fetch_select4(this.value);" name="catid">
    ...
    $query = mysqli_query($con,"SELECT 
      DISTINCT vehicle.unit,unitcat.Unitcat 
      FROM vehicle
...
<select id="batch" class="form-control" name="batch">
    <option value="">Select batch first</option>
             ...
             $qu=mysqli_query($con," SELECT * from trade");
                ...
                <div class="form-group">
 <label for="subcategoryname" class="col-sm-2 control-label form-label">Requirement</label>
 <div class="col-sm-10">
               <input type="text" name="require"></input>
</select>
</div>
</div>
