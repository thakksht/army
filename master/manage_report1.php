    ...
    <div class="col-md-12">
      <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
          ...
          $qu=mysqli_query($con,"select distinct fault.unitcat,unitcat.Unitcat from fault 
            INNER JOIN unitcat ON fault.unitcat=unitcat.id") ;                    
                            if(mysqli_num_rows($qu)>0)
                           { 
                            while($res1=mysqli_fetch_assoc($qu)){
                            //  $Id =$res1['Id'];
                            //$GR=$res['GR'];
                  $unitcat= $res1['unitcat'];
                  $Unitcat= $res1['Unitcat'];
               ...
               if(@$unit!=''){
                   ...
                   $q2=mysqli_query($con, "SELECT COUNT(*)FROM `batch` where unitcat ='$unitcat' ") ; 
               }
       ...
       if(@$unit!=''){
               ...
               // echo  "SELECT COUNT(*)FROM `fault` where fault='s' ";
                $q2=mysqli_query($con, "SELECT COUNT(*) as total FROM `batch` where unitcat ='$unitcat' ") ;
                $total=mysqli_fetch_assoc($q2);
                $query1=mysqli_query($con, "SELECT COUNT(*) as total FROM `fault` where fault='NMC' and  unitcat ='$unitcat' ") ;
                $total2=mysqli_fetch_assoc($query1);
                 $q1=mysqli_query($con, "SELECT COUNT(*) as total FROM `fault` where fault='PMC' and unitcat ='$unitcat' ") ;
                 $total3=mysqli_fetch_assoc($q1);
               ...
               $pcm=$res11['COUNT(*)'] ;
                    ...
                    ?>
                      ...
                      <tr>
                          ...
                          <td><a href="unit_listing.php?unitcat=<?php echo $unitcat ?>"><?php   echo $product?></a></td>
