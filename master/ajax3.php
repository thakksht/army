   ...
   // if($rowCount > 0){
    if(mysqli_num_rows($query)>0)
       ...
       // while($row = $query->fetch_assoc()){ 
            ...
            echo '<option name="catid" value="'.$row['Id'].'">'.$row['Brigade'].'</option>';
        }
    }else{
...
elseif(!empty($_POST["vehicle_id"])){
    //Fetch all city data
  //  $query1 = $db->query("SELECT * FROM cities WHERE state_id = ".$_POST['state_id']." AND status = 1 ORDER BY city_name ASC");
      $query1 =mysqli_query($con,"SELECT * FROM vehicle WHERE Id ='".$_POST['vehicle_id']."'");
      echo "SELECT * FROM vehicle WHERE vehicle ='".$_POST['vehicle_id']."'";
    //Count total number of rows
  //  $rowCount = $query->num_rows;
     $rowcount1=mysqli_num_rows($query1);
    //City option list
