...
$date=date("Y/m/d");
  $q = mysqli_query($con,"UPDATE `trade` SET `trade`='$Brigade2'WHERE Id='$ii'");

   if($q){
    echo '<script>alert("Update Successfully.");window.location.assign("manage_trade.php");</script>';
  }
  else{
      echo '<script>alert("please Add Again....");window.location.assign("edit_trade.php");</script>';
  }
}
?> 
...
$date=date("Y/m/d");
  $q = mysqli_query($con,"SELECT * from trade Where Id='$unitcat'");
