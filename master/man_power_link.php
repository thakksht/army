  ...
  <div class="page-header">
    <h1 class="title">Batch</h1>
      <ol class="breadcrumb">
        ...
        <li class="active">Manpower Grouping</li>
      </ol>
...
//echo "INSERT INTO `recovery`(`unit`, `vehicle`,`product`, `natureCAS`, `timeCAS`, `typerecovery`, `REC`, `timecompletion`, `GR`, `remark`, `isactive`) values('$unit','$vehicle','$product','$natureCAS','$timeCAS','$typerecovery','$REC','$timecompletion','$GR','$remark','$isactive')";
 //die("fgfd");
  $q = mysqli_query($con,"INSERT INTO `manpower_link`(`cat_unit`,  `date`,`unit`,`vehicle`,`batch`, `trade`, `name`, `isactive`) values('$unit','$date','$category','$vehicle','$batch','$trade','$name','1')");
  if($q){
    echo '<script>alert("Add Successfully.");window.location.assign("manage_manpower_link.php");</script>';
  }
  else{
...
<select id="country"  class="form-control" onchange="fetch_select4(this.value);" name="catid">
    ...
    $query = mysqli_query($con,"SELECT 
      ...
      FROM Brigade 
      INNER JOIN unitcat ON Brigade.unitcategory=unitcat.Id");
