...
<div class="content">
  ...
  <div class="page-header">
    <h1 class="title">Edit Spare Request</h1>
      <ol class="breadcrumb">
        <li><a href="dashboard.php">Dashboard</a></li>
        <li class="active">Edit Spare Request</li>
      </ol>
...
$date=date("Y/m/d");
  $q = mysqli_query($con,"UPDATE `sparerequest` SET `Unitcat`='$Unitcategory', `Unit`='$Brigade2',`Product`='$product',`Equipment`='$Brigade',`Request`='$quantity',`time`='$time', `date`='$date' WHERE Id='$ii'");
//echo "UPDATE `sparerequest` SET `Unitcat`='$Unitcategory', `Unit`='$Brigade2',`Product`='$product',`Equipment`='$Brigade',`Request`='$quantity',,`time`='$time', `date`='$date' WHERE Id='$ii'";die();
   if($q){
    echo '<script>alert("Update Successfully.");window.location.assign("manage_sparerequest.php");</script>';
  }
  else{
      echo '<script>alert("please Add Again....");window.location.assign("edit_spare_request.php");</script>';
  }
}
?> 
...
$date=date("Y/m/d");
  $q = mysqli_query($con,"SELECT
                     sparerequest.Id as fff,
                     unitcat.Unitcat,
                     brigade.Brigade,
                     sparerequest.Unitcat as Unitcat_id,
                     sparerequest.Unit as Unit_id,
                     sparerequest.Product as Product_id,
                     sparerequest.Equipment as Equipment_id,
                     sparerequest.time,
                     ...
                     Product.Product 
                     From sparerequest
                     LEFT JOIN Product ON sparerequest.Product=Product.Id
                     LEFT JOIN unitcat ON sparerequest.Unitcat=unitcat.id
                     LEFT JOIN brigade ON sparerequest.Unit=brigade.Id
                     LEFT JOIN subproduct ON sparerequest.Equipment=subproduct.Id
  Where sparerequest.Id='$unitcat'");
...
<label for="subcategoryname" class="col-sm-2 control-label form-label">Equipment</label>
 ...
 ?>
                ...
                <div class="form-group">
                  <label for="subcategoryname" class="col-sm-2 control-label form-label">Request</label>
                  <div class="col-sm-10">
                    <input type="text" value="<?php echo $request; ?>" name = "quantity" id="quantity" required>
                  </div>
                </div>

                 <div class="form-group">
                  <label for="subcategoryname" class="col-sm-2 control-label form-label">Time</label>
                  <div class="col-sm-10">                    
                    <select class="selectpicker1" name="time">
