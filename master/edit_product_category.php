...
$date=date("Y/m/d");
  $q = mysqli_query($con,"UPDATE `category_product` SET `cat_prod`='$Brigade2', `date`='$date' WHERE Id='$ii'");
//echo "UPDATE brigade SET unitcategory='$Unitcategory' and Brigade='$Brigade2' WHERE Id='$ii'";die();
   if($q){
    echo '<script>alert("Update Successfully.");window.location.assign("manage_category_product.php");</script>';
  }
  else{
      echo '<script>alert("please Add Again....");window.location.assign("edit_product_category.php");</script>';
  }
}
?>
...
$date=date("Y/m/d");
  $q = mysqli_query($con,"SELECT * from category_product Where id='$unitcat'");
  
  $res=mysqli_fetch_array($q);
$Id = $res['id'];
$Brigade = $res['cat_prod'];
?>
           <div class="panel-body">
              <form accept-charset="UTF-8" role="form" method="post" class="form-horizontal" >
               
                <div class="form-group">
                  <label for="category" class="col-sm-2 control-label form-label">Product Category</label>
                  <div class="col-sm-8">
